...
export default function AddVehicleTrimModal({
    ...
    const [data, setData] = useState({
        ...
        airportSeatingCapacity:
            (vehicleTrimModal?.isEdit && selectedVehicleTrim?.airportSeatingCapacity) || "",
        normalSeatingCapacity:
            (vehicleTrimModal?.isEdit && selectedVehicleTrim?.normalSeatingCapacity) || "",
    });
    const [error, setError] = useState("");
    const [isLoading, setIsLoading] = useState(false);

    const wrapperRef = useRef();
    useHandleClickOutside(wrapperRef, () => setVehicleTrimModal({ isEdit: false, isOpen: false }));
            ...
            >
                <div className="flex items-center justify-between border-b p-4">
                    <h2 className="font-medium">
                        {vehicleTrimModal?.isEdit ? "Update Vehicle Trim" : "Add Vehicle Trim"}
                    </h2>
                    <button
                        className="h-auto bg-transparent text-textColor text-xl"
                        onClick={() => setVehicleTrimModal({ isOpen: false, isEdit: false })}
                    >
                        <MdClose />
                    </button>
                </div>
