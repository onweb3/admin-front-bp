...
export default function HomeBannerB2bPage() {
  ...
  const fetchSections = async () => {
    try {
      setIsLoading(true);
      const response = await axios.get(`/frontend/b2b/home/banner/all/${id}`, {
  ...
  return (
    <div>
      <div className="bg-white flex items-center justify-between gap-[10px] px-6 shadow-sm border-t py-2">
        <h1 className="font-[600] text-[15px] uppercase"> Section Banners</h1>
                  ...
                  {banners?.map((section, index) => {
                    return (
                      ...
                      >
                        ...
                        <td className="p-3">{section?.link}</td>
                        <td className="p-3">
                          <div className="flex items-center gap-[15px]">
                            <img
                              src={
                                section?.image
                                  ? config?.SERVER_URL + section?.image
                                  : ""
                              }
