...
function HotelContractPage() {
    ...
    const fetchContracts = async () => {
        try {
            ...
            const response = await axios.get(
                `/hotels/contracts/single/hotel/${id}?skip=${filters?.skip}&limit=${filters?.limit}&searchQuery=${filters?.searchQuery}`,
                {
    ...
    const fetchContractGroups = async () => {
        try {
            const response = await axios.get(`/hotels/contract-groups/all/names/hotel/${id}`, {
                headers: { authorization: `Bearer ${jwtToken}` },
            });
            setContractGroups(response.data);
        } catch (err) {
            console.log(err);
        }
    };
    ...
    useEffect(() => {
        setSelectedSection("contracts");
    }, []);

    return (
        <div>
            <div className="flex items-center justify-between border-b border-dashed p-4">
                <h1 className="font-medium">Hotel Contracts</h1>
                <div className="flex items-center gap-3">
                    ...
                    {/* <Link to={`/hotels/${id}/contracts/add`}>
                        ...
                        <table className="w-full">
                            <thead className="bg-[#f3f6f9] text-grayColor text-[14px] text-left">
                                <tr>
                                    ...
                                    <th className="font-[500] p-3">Contract Group</th>
                                </tr>
                            </thead>
...
export default HotelContractPage;
