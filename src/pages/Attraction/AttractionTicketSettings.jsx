...
export default function AttractionTicketSettingPage() {
  ...
  const fetchTicketThemes = async () => {
    try {
      setIsLoading(true);
      const response = await axios.get(`/attractions/tickets/setting/all`, {
                ...
                <tbody className="text-sm">
                  {themes.samples?.map((sample, index) => {
                    return (
                      ...
                      >
                        {" "}
                        <td className="p-3">{sample?.name}</td>
                        <td className="p-3">
                          <BsEyeFill
                            onClick={(e) => {
                        ...
                        <td className="p-3">
                          <div className="flex gap-[10px]">
                            <Toggle
                              ...
                              value={
                                sample?.name === themes.selected ? true : false
                              }
                              onChange={(e) => {
                                handleDataChange(sample?.name);
                              }}
                            />
                          </div>
                        </td>
