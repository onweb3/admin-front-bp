...
export default function SectionsSettingsB2cPage() {
  ...
  const fetchSectionSettings = async () => {
    try {
      ...
      const response = await axios.get("/frontend/b2c/home/sections", {
  ...
  const addToTopAttractions = () => {
    if (selectedTopAttraction) {
      ...
      setData((prev) => {
        return {
          ...prev,
          topAttractions: [...prev.topAttractions, attractions[objIndex]],
        };
      });
    }
  };
  ...
  const removeTopAttraction = (index) => {
    const filteredAttractions = data.topAttractions?.filter((_, attrIndex) => {
  ...
  const handleSectionUpdate = async () => {
    try {
      ...
      await axios.patch(
        `/frontend/b2c/home/sections/update`,
  ...
  return (
    <div>
      <div className="bg-white flex items-center justify-between gap-[10px] px-6 shadow-sm border-t py-2">
        <h1 className="font-[600] text-[15px] uppercase">Sections Setting</h1>
          ...
          <div className="bg-white rounded p-6 shadow-sm">
            <div className="flex items-center gap-[10px]">
              <input
                ...
                checked={data.isBestSellingAttractionsSectionEnabled || false}
                onChange={(e) => {
                  setData((prev) => {
                    return {
                      ...prev,
                      isBestSellingAttractionsSectionEnabled: e.target.checked,
                    };
                  });
                }}
              />
            ...
            <div className="mt-4">
              ...
              <div className="flex max-w-[500px] items-center gap-[10px]">
                <select
                  ...
                  onChange={(e) => setSelectedBestAttraction(e.target.value)}
                  id=""
                >
                  <option value="" hidden>
                    Select Attraction
                  ...
                  {attractions?.map((attraction, index) => {
                    return (
                      <option value={attraction?._id} key={index}>
                        {attraction?.title}
                      </option>
                    );
                  })}
                </select>
                <button className="w-[200px]" onClick={addToBestAttractions}>
                  Add Attraction
                </button>
              </div>
            </div>
            ...
            <div className="mt-4">
              <div className="flex gap-[20px] overflow-x-auto">
                {data.bestSellingAttractions?.map((attraction, index) => {
                  return (
                    <div key={index} className="w-[200px] min-w-[200px]">
                      <div className="group relative w-full aspect-video rounded overflow-hidden">
                        ...
                        <div
                          ...
                          onClick={() => removeBestAttraction(index)}
                        >
                          <MdDelete />
                        </div>
                      </div>
            ...
            <div className="flex items-center gap-[10px] mt-6">
              <input
                type="checkbox"
                className="w-[16px] h-[16px]"
                checked={data.isTopAttractionsSectionEnabled || false}
                onChange={(e) =>
                  setData((prev) => {
                    return {
                      ...prev,
                      isTopAttractionsSectionEnabled: e.target.checked,
                    };
                  })
                }
              />
            ...
            <div className="mt-4">
              ...
              <div className="flex max-w-[500px] items-center gap-[10px]">
                <select
                  ...
                  onChange={(e) => setSelectedTopAttraction(e.target.value)}
                  id=""
                >
                  <option value="" hidden>
                    Select Attraction
                ...
                <button className="w-[200px]" onClick={addToTopAttractions}>
                  Add Attraction
                </button>
              </div>
            </div>
            ...
            <div className="mt-4">
              <div className="flex gap-[20px] overflow-x-auto">
                {data.topAttractions?.map((attraction, index) => {
                  return (
                    <div key={index} className="w-[200px] min-w-[200px]">
                      <div className="group relative w-full aspect-video rounded overflow-hidden">
                        <img
                        ...
                        <div
                          className="hidden group-hover:flex absolute inset-0 items-center justify-center bg-[#0007] text-red-500 text-xl cursor-pointer transition-all"
                          onClick={() => removeTopAttraction(index)}
                        >
                          <MdDelete />
                        </div>
                      </div>
