...
function EditBannerB2cPage() {
  ...
  const fetchBanner = async () => {
    try {
      ...
      if (id != null) {
        const response = await axios.get(`/frontend/b2c/banners/single/${id}`, {
  ...
  return (
    <div>
      <div className="bg-white flex items-center justify-between gap-[10px] px-6 shadow-sm border-t py-2">
        <h1 className="font-[600] text-[15px] uppercase">Edit Banner</h1>
                        ...
                        {data.banners?.map((banner, index) => {
                          return (
                            ...
                            >
                              ...
                              <td className="p-3">{banner?.title || "N/A"}</td>
                              <td className="p-3">{banner?.body || "N/A"}</td>
                              <td>
                                <img
                                  src={
                                    import.meta.env.SERVER_URL + banner?.image
                                  }
                                  alt=""
