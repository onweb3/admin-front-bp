...
export default function VehicleTypePage() {
  ...
  const deleteCategory = async (id) => {
    try {
      ...
      if (isConfirm) {
        await axios.delete(`/transfers/vehicles/vehicle-type/delete/${id}`, {
          headers: { authorization: `Bearer ${jwtToken}` },
        });
  ...
  return (
    <div>
      <div className="bg-white flex items-center justify-between gap-[10px] px-6 shadow-sm border-t py-2">
        <h1 className="font-[600] text-[15px] uppercase">Vehicle Types</h1>
                    ...
                    {categories?.map((category, index) => {
                      return (
                        ...
                        >
                          ...
                          <td className="p-3">{category?.normalOccupancy}</td>
                          <td className="p-3">
                            {category?.airportOccupancy}
                          </td>{" "}
                          <td className="p-3">
                            <img
                              src={import.meta.env.SERVER_URL + category?.image}
                              alt=""
