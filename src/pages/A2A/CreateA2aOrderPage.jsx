...
export default function CreateA2aOrder() {
  ...
  const fetchResellers = async () => {
    try {
      const response = await axios.get(`/orders/attraction/list/resellers`, {
        headers: { authorization: `Bearer ${jwtToken}` },
      });
  ...
  const fetchA2aTickets = async () => {
    try {
      ...
      const reduceTicket = response.data?.a2aSingleList?.map((data) => {
        return {
          ...data,
          fieldName: `${data.airlineOnward}(${formatDate(data?.onwardDate)}-${
            data.takeOffTimeOnward
          })-${data.airlineReturn}(${formatDate(data?.returnDate)}-${
                  ...
                  <div>
                    ...
                    <div className="">
                      <SelectDropdown
                        ...
                        setData={setTickets}
                        displayName={"fieldName"}
                        selectedData={data?.selectedTicket}
                        setSelectedData={(value) =>
                          handleSingleChange({
                            name: "selectedTicket",
                            value: value,
              ...
              data?.selectedDetails && (
                <>
                  <div className="w-full h-[40px] bg-blue-900 mt-10 flex items-center">
                    <h2 className="text-white p-5">Order Details</h2>
                  </div>
                        ...
                        return (
                          <div className="p-5">
                            ...
                            <div className="grid grid-cols-3  gap-2">
                              <div>
                                <label htmlFor="title">Mr/Mrs</label>
                                <select
                                  name="title"
                              ...
                              <div className="flex flex-col items-center justify-center gap-2">
                                <label htmlFor="">Is Infant</label>
                                <input
                                  type="checkbox"
                                  name="isInfant"
                                  defaultChecked={passenger?.isInfant === true}
                                  onChange={(e) => {
                                    handlePassengerChange(e, index);
                                  }}
                                  // required
                              ...
                              {passenger.isInfant && (
                                <>
                                  ...
                                  <div>
                                    ...
                                    <select
                                      name="title"
                                      value={passenger?.infant?.title || ""}
                                      onChange={(e) => {
                                        handleInfantChange(e, index);
                                      }}
                                      id="title"
                                      className="capitalize"
                                    >
                                  ...
                                  <div>
                                    ...
                                    <input
                                      ...
                                      placeholder="Enter  name"
                                      name="firstName"
                                      value={passenger?.infant?.firstName || ""}
                                      onChange={(e) => {
                                        handleInfantChange(e, index);
                                      }}
                                      required
                                  ...
                                  <div>
                                    <label htmlFor="">Last Name (Infant)</label>
                                    <input
                                      type="text"
                                      placeholder="Enter  name"
                                      name="lastName"
                                      value={passenger?.infant?.lastName || ""}
                                      onChange={(e) => {
                                        handleInfantChange(e, index);
                                      }}
                                      required
                                  ...
                                  <div>
                                    <label htmlFor="">Passport (Infant)</label>
                                    <input
                                      type="text"
                                      placeholder="Enter passport Number"
                                      name="passportNo"
                                      value={
                                        passenger?.infant?.passportNo || ""
                                      }
                                      onChange={(e) => {
                                        handleInfantChange(e, index);
                                      }}
                    ...
                    <div className="p-8 flex justify-center max-h-[470px]">
                      <div class="max-w-sm rounded overflow-hidden shadow-lg">
                        <div class="px-6 py-4 w-full">
                          <div>
                            Account Balance : {wallet?.balance.toFixed(2)} AED
                          </div>
                          <div className="p-2 bg-gray-50">
                            <div className="font-semibold text-base my-2 flex items-center justify-center gap-3">
                              ...
                              <span className="font-light text-base">
                                {data?.selectedDetails?.airlineOnwardNo}
                              </span>
                            </div>{" "}
                            <div class=" mb-2 flex gap-2 items-center ">
                              <span>
                                {" "}
                                {data?.selectedDetails?.airlineOnward}
                              </span>
                              <span className="px-2">
                                {formatDate(data?.selectedDetails?.onwardDate)}
                              </span>
                              <span>
                                {data?.selectedDetails?.takeOffTimeOnward}
                              </span>
                            </div>
                            <div className="font-semibold text-base my-2 flex items-center justify-center gap-3">
                              ...
                              <span className="font-light text-base">
                                {data?.selectedDetails?.airlineReturnNo}
                              </span>
                            </div>{" "}
                            <div class="font- text-base mb-2 flex gap-2 items-center justify-center ">
                              <span>
                                {" "}
                                {data?.selectedDetails?.airlineReturn}
                              </span>
                              <span className="px-2">
                                {formatDate(data?.selectedDetails?.returnDate)}
                              </span>
                              <span>
                                {data?.selectedDetails?.takeOffTimeReturn}
                              </span>
                            </div>
                          </div>
                          ...
                          <div class="text-gray-700 text-base flex gap-4  items-center justify-between mb-2 ">
                            <div className="flex font-bold text-sm">Ticket</div>
                            <div className="flex font-bold text-sm">Count</div>
                            <div className="flex font-bold text-sm">Price</div>
                            <div className="flex font-bold text-sm">Total</div>
                          </div>
                          {data?.totalPax > 0 && (
                            <div class="text-gray-700 text-base flex gap-4  items-center justify-between ">
                              <div className="flex">Adult</div>
                              <div className="flex">{data?.totalPax}</div>
                              <div className="flex">
                                {data?.selectedDetails?.price}
                              </div>
                              <div className="flex">
                                {data?.selectedDetails?.price * data?.totalPax}
                              </div>
                            </div>
                          )}
                          ...
                          {infantCount > 0 && (
                            <div class="text-gray-700 text-base flex gap-4  items-center justify-between ">
                              <div className="flex">Infant</div>
                              <div className="flex">{infantCount}</div>
                              <div className="flex">
                                {data?.selectedDetails?.infantPrice}
                              </div>
                              <div className="flex">
                                {data?.selectedDetails?.infantPrice *
                                  infantCount}
                          ...
                          <div className="flex justify-end ">
                            {data?.selectedDetails?.price * data?.totalPax +
                              data?.selectedDetails?.infantPrice * infantCount}
                          </div>
                        </div>
                        <div class="px-6 pb-2">
